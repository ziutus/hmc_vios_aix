...
def start_system_and_check_progress(frame_name)
  ...
  puts 'Checking status of Frame'
	frame_status = $execute.execute(frame.statusCheck(), 'on').gsub("\n", '')
	puts frame_status.to_s + 'Frame startTime: ' + frame.startTime.to_s
...
case action
  ...
  when 'statusAll'
    ...
    puts frame.dataGet()
    DataString = $execute.execute(frame.dataGet())
  ...
  when 'status'
		...
		puts $execute.execute(frame.statusCheck())

  when 'off'
		puts 'Will power off the frame'
		puts frame.stop
		puts $execute.execute(frame.stop)
